 ...
 * Desc:
 */
class NewsFunction : Function<ResponseBody, BaseListResponse<NewsListBeanMvp>> {

    companion object {
        private const val TAG = "Function"
    }

    override fun apply(responseBody: ResponseBody): BaseListResponse<NewsListBeanMvp> {
    ...
    private data class OriginBean(
        val code: Int,
        val message: String,
        val result: MutableList<NewsListBeanMvp>?
    ) : Serializable
