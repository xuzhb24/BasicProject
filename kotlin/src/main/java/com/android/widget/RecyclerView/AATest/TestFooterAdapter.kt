 ...
 * Desc:尾部Footer
 */
class TestFooterAdapter(context: Context, list: MutableList<String>, private var mSelectedList: MutableList<String>) :
    BaseAdapter<String>(context, list, object : MultiViewType<String> {
        override fun getLayoutId(item: String, position: Int, totalCount: Int): Int {
            return when (position) {
                totalCount - 1 -> R.layout.item_header
    ...
    override fun bindData(holder: ViewHolder, data: String, position: Int) {
        when (position) {
            itemCount - 1 -> holder.setText(R.id.count_tv, data)
            else -> {
